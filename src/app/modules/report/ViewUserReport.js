...
import { viewTrainingProgress } from "./redux/functions";

function ViewUserReport(props) {
  let { id } = useParams()
                ...
                {selectedValue.map((val, index) =>
                  ...
                  <div className="timeline-item" key={index}>
                    ...
                    <div className="timeline-desc timeline-desc-light-primary">
                      <span className="svg-icon-primary svg-icon-md" >
                        {`Course Name - `} <b>{val.training[0].trainingname}</b>
                      </span>
                    </div>
                    <div className="timeline-desc timeline-desc-light-primary">
                      <span className="svg-icon-primary svg-icon-md" >
                        {`Course Level -`} <b>{val.training[0].level}</b>
                      </span>
                    </div>
                    <div className="timeline-desc timeline-desc-light-primary">
...
export default connect(mapStateToProps, {})(ViewUserReport);
